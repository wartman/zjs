/**
 * z.build 0.0.2
 */

var z = require('../dist/z').z;
var imports = require('../dist/z').imports;
var define = require('../dist/z').define;
var UglifyJS = require("uglify-js");
var fs = require('fs');

/**
 * Overwrite z.module's plugins for use in the node env.
 */

/**
 * Mock script loading.
 */
var ScriptMock = z.Loader.extend({
  load: function(req){
    var contents = fs.readFileSync(req.url, 'utf-8')
      // Wrap the contents in a function that we can pass stuff to.
      // This is done to get around the fact that node doesn't have any globals,
      // so z will be undefined if we try to load it normally.
      , zModule;

    try{
      zModule = Function('z, imports, define', contents);
      zModule(z, imports, define);
    } catch(e) {
      console.log('  ...building failed, wrapping file');
      // Stick it in a wrapper and hope for the best!
      zModule = Function('z, imports, define', 'z(function(){'+ contents +'});');
      zModule(z, imports, define);
    }

    this._resolve(true, 1);
  }
}); 

z.plugin('script', ScriptMock, function(req, res, next, error){
  var name = req.from;
  console.log('  ...building: ', name);
  
  z.ensureModule(name);
  next();
});

/**
 * Mock ajax loading
 */
AjaxMock = z.Loader.extend({
  load: function(req){
    var name = req.from
      , contents = fs.readFileSync(req.url, 'utf-8');

    this._value = contents;

    this._resolve(contents, 1);
  }
});

z.plugin('ajax', AjaxMock, function(req, res, next, error){
  var name = req.from;

  console.log('  ...added file: ', name);

  if(res){
    var factory = "z('"+name+"').exports(function(){ return '" + escape(res) + "'; });";
    var zModule = Function('z, imports, define', factory);

    // Run the module.
    zModule(z, imports, exports, define);
  } else {
    console.log('  Warning: result was empty. Continuing, but be sure this is desired...')
  }

  next();
}, {
  ext: 'txt'
});

var escapes = {
      "'" : "'",
      '\\': '\\',
      '\r': 'r',
      '\n': 'n',
      '\t': 't',
      '\u2028': 'u2028',
      '\u2029': 'u2029'
    }
  , escaper = /\\|'|\r|\n|\t|\u2028|\u2029/g;
var escape = function(txt){
  return txt.replace(escaper, function(match) { return '\\' + escapes[match]; });
}

/**
 * ----------------------------------------------------------------------
 * z.Build
 *
 * Fit's module-compiler, used to crunch things into a single file.
 * 
 * Fit projects ideally should be built before they are made live --
 * the module system should be used mainly as a development tool.
 */
var Build = function(options){

  var defaults = {
    optimize: false,
    main: 'main',
    buildfile: false,
    shim: {
      'jquery' : function(mod){
        var compile = mod._factory.toString();
        return '/*jquery*/\njquery: (' + compile + ')(),\n\n'
      }
    }
  };

  this.options = z.u.defaults(defaults, options);
}

Build.prototype.setup = function(options){
  this.options = z.u.defaults(this.options, options);
}

Build.prototype.start = function(main, dest, next){
  var self = this
    , mainName = main.replace('.js', '').replace(/\//g, '.');

  if(this.options.buildfile){
    try{
      var options = require(main)
        , paths = z.u.extract(['main', 'dest'], options);
      main = paths.src;
      dest = paths.dest;
      this.setup(options);
    } catch(e){
      console.log('Could not find a buildz.js file.\nTry providing a main file (e.g. \'main.js\') or make sure you are in the correct dir.')
      return false;
    }
  }

  z.setup({
    env: 'server'
  });

  console.log('\nbuilding...\n');

  var loader = new ScriptMock({url: main});

  loader.done(function(){
    if(self.options.main){
      mainName = self.options.main;
    }

    z.ensureModule(mainName);

    console.log('  ...main module found');
    console.log('  ...added: ', mainName);

    if(self.options.project === undefined){
      self.options.project = main.replace('.js', '').replace(/\//g, '.');
    }

    z(mainName).done(function(){
      var rendered = self.render(dest);
      if(z.u.isFunction(next)){
        next(rendered);
      }
    });
  }, function(e){
    console.error(e);
  });

  
}

Build.prototype.compile = function(name){

  if(false === z.has(name)){
    console.log('   The module '+ name + 'was loaded, but is not in the registry.')
    console.log('   continuing...');
    console.log();
    return;
  }

  var mod = z(name)
    , rendered = ''
    , deps = mod._deps
    , factory;

  if(mod._factory){
    factory = mod._factory.toString();
  } else {
    console.log('   Warning: factory not found for module: '+ name);
    console.log('   Continuing...')
  }

  if(this.options.shim.hasOwnProperty(name)){
    rendered = this.options.shim[name](mod);
    return rendered;
  }

  // Build the compiled string.
  rendered = '/*'+name+'*/\n';
  rendered += "z('"+name+"').\n";

  // Build imports.
  if(deps.length > 0){
    var imps = [];
    z.u(deps).each(function(dep){
      var imp = 'imports(';
      imp += "'" + dep.from + "'"
      if(dep.uses){
        if(z.u.isArray(dep.uses)){
          imp += ", ['" + dep.uses.join("', '") + "']"; 
        } else {
          imp += ", '" + dep.uses + "'";
        }
      } else {
        imp += ", '*'"
      }
      if(dep.options && dep.options.type !== 'script'){
        imp += ", " +  JSON.stringify(dep.options);
      }
      imp += ").\n";
      imps.push(imp);
    });
    rendered += imps.join('');
  }

  rendered += 'exports(' + factory + ');\n\n';

  return rendered;
}

Build.prototype.renderZ = function(){
  // Eventually make this configurable.
  var root = __dirname + '/../'
    , rendered = '';

  z.u([
    "src/util_large.js",
    // "src/util_small.js",
    "src/core.js",
    "src/Class.js",
    "src/Module.js",
    "src/Plugable.js",
    "src/Loader.js", // Should be optional
    "src/Script.js", // Should be optional
    "src/Ajax.js",   // Should be optional
    "src/amd.js",
    "src/plugins.js"
  ]).each(function(file){
    rendered += fs.readFileSync(root + file, 'utf-8') + "\n\n";
  });

  rendered = '\n/*! zjs */\n' + UglifyJS.minify(rendered, {fromString: true}).code;

  return rendered;
}

Build.prototype.render = function(dest){

  var self = this
    , rendered = '';

  // Check to see if the user defined any build settings in
  // z.setup. This could include compile shims, destinations, etc.
  if(z.config.build){
    z.u.extend(this.options, z.config.build);
    delete z.config.build;
  }

  dest = (dest || this.options.dest);

  if(!dest){
    console.log('No destination!');
    console.log('Set [dest] in one of the following ways:');
    console.log('   console: $ zjs build [src] [dest]');
    console.log('   in main.js: z.setup({ ... build:{ ... dest:[dest]... } ... });');
    console.log('   in zbuild.js: ... dest:[dest] ...');
    console.log('No files have been modified.');
    return;
  }

  z.u.each(z.modules, function(mod, name){
    rendered += self.compile(name);
  });

  rendered = '/* modules */\n\n' + rendered + '\n';

  delete z.config.env;

  // Add the config
  rendered = '\n\nz.setup(' + JSON.stringify(z.config) + ');\n\n' + rendered;

  // Add z itself to the top of the compiled project
  rendered = '(function(root){\n' + this.renderZ() + rendered + '\n})(window);';

  if(this.options.optimize){
    console.log('\nuglifying...');
    rendered = UglifyJS.minify(rendered, {fromString: true}).code;
    console.log('\n  ...uglified');
  }

  if(this.options.about){
    this.options.project += '\n' + this.options.about;
  }

  rendered = '/*'+this.options.project+'*/\n' + rendered;

  fs.writeFileSync(dest, rendered);

  console.log('\nbuilt to '+ dest + '\n');

  if(typeof next == 'function'){
    next(dest, rendered);
  }

  return rendered;
}

// Export instance of build.
exports = module.exports = new Build;

// Export Build
exports.Build = Build
