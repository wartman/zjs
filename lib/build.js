 ...
 * @param {String} name
 * @param {String} before
 * @param {String} esc String to escape
 * @param {String} after
 */
z.builder = function(name, before, esc, after){
  // This feels clunky. A better way???
  _contents[name] = before + escape(esc) + after;
}
...
var ScriptMock = z.Resolver.extend({
  ...
  load: function(req){
    ...
    // Save the actual contents of the file for later
    _contents[req.from] = contents;
...
z.loader('ajax')
  ...
  .build(function(req, res, builder){
    builder(req.from, "z('"+name+"').exports(function(){ return '", res, "'; });");
  });
 ...
 */
var _annon = /z\(\)/;
var _annonCallback = /z\(\s?function/;
Build.prototype.compile = function(name){
  ...
  if(_annon.test(content)){
    content = content.replace(_annon, "z('" + name + "')"); 
  } else if (_annonCallback.test(content)){
    content = content.replace(_annonCallback, "z('" + name + "', function");
  }
