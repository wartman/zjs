...
/**
 * Deffered is the primary API
 */
var Deffered = z.Class({
...
var Promise = z.Class({
  ...
  _resolve: function(val){
    ...
    if(u.isObject(val) || u.isFunction(val)){
      var then;
      try{
        then = val.then;
      } catch(e) {
  ...
  _enqueue: function(defer, onFulfilled, onRejected, onProgress, ctx){
    if(onRejected && !u.isFunction(onRejected)){
      ctx = onRejected;
      onRejected = undefined;
    }
    if(onProgress && !u.isFunction(onProgress)){
      ctx = onProgress;
    ...
    if(!this.isRejected()){
      cb = { 
        defer: defer,
        fn: u.isFunction(onFulfilled)? onFulfilled : undefined,
        ctx: ctx
      };
...
Promise.when = function(value, onFulfilled, onRejected, onProgress, ctx){
  return Promise.cast(value).then(onFulfilled, onRejected, onProgress, ctx);
}
