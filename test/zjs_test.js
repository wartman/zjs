(function(){
  ...
  test('Modus-style API', function () {
    stop();
    z('moduleTest.modusExport').exports({foo:'foo'});
    z('moduleTest.modus', function (module) {
      module.imports('moduleTest.modusExport');
      module.exports(function () {
        start();
        equal(moduleTest.modusExport.foo, 'foo', 'Modules imported, alternate API works.');
      });
    });
  });

  test('Export items by name', function () {
    stop();
    z('moduleTest.exportItems', function (module) {
      ...
      module.exports('bin', function () {
        return function () { return moduleTest.exportItems.bar.foo; };
      });
      module.exports(function () {
        return function () { return 'root'; };
      });
    }).done(function () {
      start();
      equal(moduleTest.exportItems(), 'root', 'Defined root export');
      equal(moduleTest.exportItems.foo, 'foo', 'Exported single item');
      deepEqual(moduleTest.exportItems.bar, {
        foo: 'foo',
        bar: 'bar'
      }, 'Exported object');
      equal(moduleTest.exportItems.bin(), 'foo', 'Exported function, used previously exported component');
    }, function (reason) {
      start();
      ok(false, reason);
    });
